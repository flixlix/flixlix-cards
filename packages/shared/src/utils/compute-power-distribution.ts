...
export function computePowerDistributionAfterSolarAndBattery(params: {
  ...
  if (solar.state.toHome !== null && solar.state.toHome < 0) {
    ...
    solar.state.toBattery =
      (solar.state.total ?? 0) - (solar.state.toHome || 0) - (grid.state.toGrid || 0);
  ...
  if (battery.has) {
    ...
    battery.state.toGrid =
      (battery.state.toGrid || 0) > largestGridBatteryTolerance ? battery.state.toGrid || 0 : 0;
  ...
  if (grid.powerOutage.isOutage) {
    grid.state.fromGrid = grid.powerOutage.entityGenerator
      ? Math.max(getEntityStateWatts(grid.powerOutage.entityGenerator), 0)
      : 0;
    grid.state.toHome = Math.max((grid.state.fromGrid ?? 0) - (grid.state.toBattery ?? 0), 0);
    grid.state.toGrid = 0;
