...
export async function getStatistics(
  ...
  Object.entries(data).forEach(([statisticId, stat]) => {
    if (!stat?.length) {
      return;
    }
    const first = stat[0]!;
    if (new Date(first.start) <= startMinHour) {
      return;
    }
    stat.unshift({
      statistic_id: first.statistic_id ?? statisticId,
      start: startMinHour.toISOString(),
      end: startMinHour.toISOString(),
      last_reset: first.last_reset,
      max: first.max,
      mean: first.mean,
      min: first.min,
      sum: 0,
