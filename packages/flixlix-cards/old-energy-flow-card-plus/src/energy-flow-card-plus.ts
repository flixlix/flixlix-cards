import { type EnergyFlowCardPlusConfig, type EntitiesConfig } from "@/energy-flow-card-plus-config";
import {
...
import { type HomeAssistantReal } from "@/hass";
...
import { type HassEntities, type HassEntity } from "home-assistant-js-websocket";
import { LitElement, type PropertyValues, type TemplateResult, html, nothing, svg } from "lit";
...
const ENTITY_INVERT_CONFIG_KEY: Record<EntityType, keyof EntitiesConfig> = {
  battery: "battery",
  grid: "grid",
  solar: "solar",
  individual1: "individual1",
  individual2: "individual2",
  individual1Secondary: "individual1",
  individual2Secondary: "individual2",
  solarSecondary: "solar",
  homeSecondary: "home",
  gridSecondary: "grid",
  nonFossilSecondary: "fossil_fuel_percentage",
};

type AnimatedLineFlowDurKey =
  | "batteryGrid"
  | "batteryToHome"
  | "gridToHome"
  | "solarToBattery"
...
export default class EnergyFlowCardPlus extends SubscribeMixin(LitElement) {
  ...
  public hassSubscribe() {
    ...
    const getEnergyDataCollectionPoll = (
      ...
      if (energyCollection) {
        ...
        console.info(getEnergyDataCollection(this.hass));
        reject(
          new Error(
  ...
  public setConfig(config: EnergyFlowCardPlusConfig): void {
    ...
    this._config = {
      ...
      max_expected_energy: coerceNumber(
        config.max_expected_energy,
        defaultValues.maxExpectedEnergy
      ),
      min_expected_energy: coerceNumber(
        config.min_expected_energy,
        defaultValues.minExpectedEnergy
      ),
    };
  ...
  private populateEntitiesArr(): void {
    ...
    (Object.keys(entities) as (keyof EntitiesConfig)[]).forEach((key) => {
      const section = entities[key];
      if (!section) {
        return;
      }
      if (typeof section.entity === "string" || Array.isArray(section.entity)) {
        if (Array.isArray(section.entity)) {
          section.entity.forEach((entityId: string) => {
            this.entitiesArr.push(entityId);
          });
        } else {
          this.entitiesArr.push(section.entity);
        }
      } else if (typeof section.entity === "object" && section.entity !== null) {
        if (Array.isArray(section.entity?.consumption)) {
          section.entity.consumption.forEach((entityId: string) => {
            this.entitiesArr.push(entityId);
          });
        } else {
          this.entitiesArr.push(section.entity?.consumption);
        }
        if (Array.isArray(section.entity?.production)) {
          section.entity.production.forEach((entityId: string) => {
            this.entitiesArr.push(entityId);
          });
        } else {
          this.entitiesArr.push(section.entity?.production);
        }
      }
    });
  ...
  private entityInverted = (entityType: EntityType): boolean => {
    const section = this._config.entities[ENTITY_INVERT_CONFIG_KEY[entityType]];
    return !!(
      section &&
      "invert_state" in section &&
      (section as { invert_state?: boolean }).invert_state
    );
  };

  private previousDur: Partial<Record<AnimatedLineFlowDurKey, number>> = {};
  ...
  private getEntityState = (entity: string | undefined, instantaneousValue?: boolean): number => {
    ...
    const stateObj =
      this._config?.energy_date_selection !== false && !instantaneousValue
        ? this.states[entity]
  ...
  private getEntityStateWatthours = (
    ...
    const valuesArr: number[] = entityArr.map((entity) => {
      ...
      if (stateObj?.attributes.unit_of_measurement?.toUpperCase().startsWith("KWH"))
        return value * 1000; // case insensitive check `KWH`
      else if (stateObj?.attributes.unit_of_measurement?.toUpperCase().startsWith("MWH"))
        return value * 1000000; // case insensitive check `MWH`
      return value;
    });
  ...
  private openDetails(
    event: { stopPropagation: any; key?: string },
  ...
  private hasField(field?: any, acceptStringState?: boolean): boolean {
    return (
      ...
      (this.getEntityStateWatthours(field?.entity) > (field?.display_zero_tolerance ?? 0) &&
      Array.isArray(field?.entity)
  ...
  private computeFieldName(field: any, fallback: string): string {
    ...
    if (field?.use_metadata)
      return this.getEntityStateObj(field.entity)?.attributes?.friendly_name || "";
    return fallback;
  }
  ...
  private getSecondaryState = (
    ...
    if (field.has) {
      ...
      const secondaryState =
        secondaryEntity && this.getEntityStateWatthours(secondaryEntity, wantsInstantaneousValue);
  ...
  protected render(): TemplateResult {
    ...
    if (!this._data && this._config.energy_date_selection !== false) {
      return html`<ha-card style="padding: 2rem">
        ${this.hass.localize("ui.panel.lovelace.cards.energy.loading")}<br />Make sure you have the
        Energy Integration setup and a Date Selector in this View or set
    ...
    const grid = {
      ...
      powerOutage: {
        ...
        isOutage:
          (entities.grid &&
            entities.grid.power_outage?.entity &&
            this.hass.states[entities.grid.power_outage.entity]?.state) ===
    ...
    const solar = {
      ...
      mainEntity: Array.isArray(entities.solar?.entity)
        ? entities.solar?.entity[0]
        : entities.solar?.entity,
    ...
    const battery = {
      ...
      state_of_charge: {
        state: entities.battery?.state_of_charge?.length
    ...
    const home = {
      ...
      mainEntity: Array.isArray(entities.home?.entity)
        ? entities.home?.entity[0]
        : entities.home?.entity,
    ...
    const getIndividualObject = (field: "individual1" | "individual2") => ({
      ...
      mainEntity: Array.isArray(entities[field]?.entity)
        ? entities[field]?.entity[0]
        : (entities[field]?.entity as string | undefined),
      ...
      icon: this.computeFieldIcon(
        entities[field],
        field === "individual1" ? "mdi:car-electric" : "mdi:motorbike-electric"
      ),
      name: this.computeFieldName(
        entities[field],
        field === "individual1" ? localize("card.label.car") : localize("card.label.motorbike")
      ),
    ...
    const nonFossil = {
      ...
      name:
        ...
        (entities.fossil_fuel_percentage?.use_metadata &&
          this.getEntityStateObj(entities.fossil_fuel_percentage.entity)?.attributes
            .friendly_name) ||
      ...
      secondary: {
        ...
        energyDateSelection:
          entities.fossil_fuel_percentage?.secondary_info?.energy_date_selection || false,
      },
    };
    ...
    if (grid.color.fromGrid !== undefined) {
      ...
      this.style.setProperty(
        ...
        grid.color.fromGrid || "var(--energy-grid-consumption-color)"
      );
    }
    ...
    if (grid.has) {
      if (typeof entities.grid!.entity === "string") {
        if (this.entityInverted("grid")) {
          grid.state.fromGrid = Math.abs(
            Math.min(this.getEntityStateWatthours(entities.grid?.entity), 0)
          );
        } else {
    ...
    if (
      entities.grid?.display_zero_tolerance !== undefined &&
      entities.grid?.display_zero_tolerance <= grid.state.fromGrid
    ) {
      grid.state.fromGrid = 0;
    }
    ...
    if (
      entities.grid?.display_zero_tolerance !== undefined &&
      entities.grid?.display_zero_tolerance <= (grid.state.toGrid ?? 0)
    ) {
      grid.state.toGrid = 0;
    }
    ...
    individual1.state = individual1.has
      ? this.getEntityStateWatthours(entities.individual1?.entity)
      : 0;
    individual2.state = individual2.has
      ? this.getEntityStateWatthours(entities.individual2?.entity)
      : 0;
    ...
    individual1.secondary.state = this.getSecondaryState(
      individual1.secondary,
      "individual1Secondary"
    );
    individual2.secondary.state = this.getSecondaryState(
    ...
    if (solar.has) {
      if (this.entityInverted("solar"))
        solar.state.total = Math.abs(
          Math.min(this.getEntityStateWatthours(entities.solar?.entity), 0)
        );
    ...
    if (battery.has) {
      ...
      battery.state.fromBattery = this.getEntityStateWatthours(
        entities.battery?.entity?.consumption
      );
    }
    ...
    const totalIndividualConsumption =
      coerceNumber(individual1.state, 0) + coerceNumber(individual2.state, 0);
    ...
    if (this._data && this._data.co2SignalEntity && this._data.fossilEnergyConsumption) {
      const highCarbonEnergy =
        Object.values(this._data.fossilEnergyConsumption).reduce((sum, a) => sum + a, 0) * 1000;
    ...
    const hasNonFossilFuelUsage =
      ...
      lowCarbonEnergy &&
      lowCarbonEnergy >
        ((entities.fossil_fuel_percentage?.display_zero_tolerance ?? 0) * 1000 || 0);

    const hasFossilFuelPercentage = entities.fossil_fuel_percentage?.show === true;

    if (this._config.energy_date_selection === false) {
      lowCarbonPercentage =
        100 - this.getEntityState(entities.fossil_fuel_percentage?.entity, true);
    ...
    const batteryChargeState = entities?.battery?.state_of_charge
      ? this.getEntityState(entities.battery?.state_of_charge, true)
      : null;
    ...
    const newDur = {
      ...
      nonFossil: this.circleRate(lowCarbonEnergy ?? 0, totalLines),
    };

    const lineFlowAnimations: { durKey: AnimatedLineFlowDurKey; svgId: string }[] = [
      { durKey: "batteryGrid", svgId: "battery-grid-flow" },
      { durKey: "batteryToHome", svgId: "battery-home-flow" },
      { durKey: "gridToHome", svgId: "grid-home-flow" },
      { durKey: "solarToBattery", svgId: "solar-battery-flow" },
      { durKey: "solarToGrid", svgId: "solar-grid-flow" },
      { durKey: "solarToHome", svgId: "solar-home-flow" },
    ];
    const root = this.shadowRoot;
    lineFlowAnimations.forEach(({ durKey, svgId }) => {
      const el = root?.querySelector(`#${svgId}`);
      const flowSVGElement = el instanceof SVGSVGElement ? el : null;
      if (
        flowSVGElement &&
        this.previousDur[durKey] &&
        this.previousDur[durKey] !== newDur[durKey]
      ) {
        flowSVGElement.pauseAnimations();
        flowSVGElement.setCurrentTime(
          flowSVGElement.getCurrentTime() * (newDur[durKey] / this.previousDur[durKey])
        );
    ...
    if (nonFossilColor !== undefined) {
      ...
      this.style.setProperty(
        "--non-fossil-color",
        nonFossilColor || "var(--energy-non-fossil-color)"
      );
    }
    this.style.setProperty(
    ...
    const homeSourceKeys = Object.keys(homeSources) as (keyof typeof homeSources)[];
    const homeLargestSource = homeSourceKeys.reduce((a, b) =>
      (homeSources[a].value ?? 0) > (homeSources[b].value ?? 0) ? a : b
    ) as keyof typeof homeSources;
    ...
    const solarIcon =
      entities.solar?.icon ||
      (entities.solar?.use_metadata &&
        this.getEntityStateObj(solar.mainEntity)?.attributes?.icon) ||
      "mdi:solar-power";

    const solarName: string =
      entities.solar?.name ||
      (entities.solar?.use_metadata &&
        this.getEntityStateObj(solar.mainEntity)?.attributes.friendly_name) ||
      this.hass.localize("ui.panel.lovelace.cards.energy.energy_distribution.solar");

    const homeIcon =
      entities.home?.icon ||
      (entities.home?.use_metadata && this.getEntityStateObj(home.mainEntity)?.attributes?.icon) ||
      "mdi:home";

    const homeName =
      entities.home?.name ||
      (entities.home?.use_metadata &&
        this.getEntityStateObj(home.mainEntity)?.attributes.friendly_name) ||
      this.hass.localize("ui.panel.lovelace.cards.energy.energy_distribution.home");
    ...
    const nonFossilName =
      entities.fossil_fuel_percentage?.name ||
      (entities.fossil_fuel_percentage?.use_metadata &&
        this.getEntityStateObj(entities.fossil_fuel_percentage.entity)?.attributes.friendly_name) ||
      this.hass.localize("ui.panel.lovelace.cards.energy.energy_distribution.low_carbon");
    ...
    this.style.setProperty(
      ...
      entities.individual1?.secondary_info?.color_value
        ? "var(--individualone-color)"
        : "var(--primary-text-color)"
    );
    this.style.setProperty(
      "--secondary-text-individualtwo-color",
      entities.individual2?.secondary_info?.color_value
      ...
      if (value || template) {
        ...
        >
          ${entities.solar?.secondary_info?.icon
            ? html`<ha-icon class="secondary-info small" .icon=${icon}></ha-icon>`
            : ""}
          ${template ?? value}</span
        >`;
      }
    ...
    const individualSecondarySpan = (individual: Individual, key: string) => {
      ...
      const passesDisplayZeroCheck =
        ...
        (isNumberValue(individual.secondary.state)
          ? (Number(individual.secondary.state) || 0) >
            (individual.secondary.displayZeroTolerance ?? 0)
          : true);
      if (!individual.secondary.has || !passesDisplayZeroCheck) {
        return nothing;
      }
        ...
        <div class="card-content" id="energy-flow-card-plus">
          ${solar.has || individual2.has || individual1.has || hasFossilFuelPercentage
            ? html`<div class="row">
                ...
                (!hasNonFossilFuelUsage && entities.fossil_fuel_percentage?.display_zero === false)
                  ...
                  : html`<div class="circle-container low-carbon">
                      ...
                      ${this.showLine(lowCarbonEnergy || 0)
                        ? html`
                  ...
                  ? html`<div class="circle-container solar">
                      ...
                      >
                        ...
                        <ha-icon
                          ...
                          ${entities.solar?.display_zero_state !== false ||
                        ...
                        (solar.state.total || 0) > 0
                          ? html` <span class="solar">
                              ${this.displayValue(solar.state.total)}</span
                            >`
                          : ""}
                      </div>
                    </div>`
                  ...
                  ? html`<div class="circle-container individual2">
                      ...
                      >
                        ...
                        <ha-icon
                          ...
                          style="${individual2.secondary.has
                            ? "padding-top: 2px;"
                            : "padding-top: 0px;"}
                          ${entities.individual2?.display_zero_state !== false ||
                          (individual2.state || 0) > 0
                            ? "padding-bottom: 2px;"
                            : "padding-bottom: 0px;"}"
                        ></ha-icon>
                        ${entities.individual2?.display_zero_state !== false ||
                              ...
                              ${individual2.state
                                ? svg`<circle
                                    ...
                                    >
                                      <animateMotion
                                        dur="${this.additionalCircleRate(
                                          entities.individual2?.calculate_flow_rate,
                                          newDur.individual2
                                        )}s"
                                        repeatCount="indefinite"
                                        calcMode="linear"
                                        keyPoints="${entities.individual2?.inverted_animation
                                          ? "0;1"
                                          : "1;0"}"
                                        keyTimes="0;1"
                                      >
                    ...
                    ? html`<div class="circle-container individual1">
                        ...
                        >
                          ...
                          <ha-icon
                            ...
                            style="${individual1.secondary.has
                              ? "padding-top: 2px;"
                              : "padding-top: 0px;"}
                          ${entities.individual1?.display_zero_state !== false ||
                            (individual1.state || 0) > 0
                              ? "padding-bottom: 2px;"
                              : "padding-bottom: 0px;"}"
                          ></ha-icon>
                          ${entities.individual1?.display_zero_state !== false ||
                                ...
                                ${individual1.state
                                  ? svg`<circle
                                      ...
                                      >
                                        <animateMotion
                                          dur="${this.additionalCircleRate(
                                            entities.individual1?.calculate_flow_rate,
                                            newDur.individual1
                                          )}s"
                                          repeatCount="indefinite"
                                          calcMode="linear"
                                          keyPoints="${entities.individual1?.inverted_animation
                                            ? "0;1"
                                            : "1;0"}"
                                          keyTimes="0;1"
                                        >
          ...
          <div class="row">
              ...
              ? html` <div class="circle-container grid">
                  ...
                  >
                    ...
                    !grid.powerOutage.isOutage
                      ...
                      : ""}
                    ${grid.powerOutage.isOutage
                      ? html`<span class="grid power-outage">
                          ${entities.grid?.power_outage?.label_alert || html`Power<br />Outage`}
                        </span>`
                      : ""}
                  </div>
            ...
            <div class="circle-container home">
              ...
              ${this.showLine(individual1.state || 0) && individual2.has
                ? ""
                : html` <span class="label">${homeName}</span>`}
            </div>
          </div>
            ...
            ? html`<div
              ...
              >
                ...
                >
                  ...
                  ></path>
                  ${solar.state.toHome
                    ? svg`<circle r="1" class="solar" vector-effect="non-scaling-stroke">
                          <animateMotion
              ...
              >
                ...
                >
                  ...
                  ${solar.state.toBattery
                    ? svg`<circle r="1" class="battery-solar" vector-effect="non-scaling-stroke">
                          <animateMotion
              ...
              >
                ...
                >
                  ...
                  ${battery.state.toHome
                    ? svg`<circle r="1" class="battery-home" vector-effect="non-scaling-stroke">
                          <animateMotion
          ...
          ${grid.has &&
          battery.has &&
          this.showLine(Math.max(grid.state.toBattery || 0, battery.state.toGrid || 0))
            ? html`<div
                class="lines ${classMap({
                  high: battery.has,
                  "individual1-individual2": !battery.has && individual2.has && individual1.has,
              ...
              >
                ...
                >
                  ...
                  ${battery.state.toGrid
                    ? svg`<circle r="1" class="battery-to-grid" vector-effect="non-scaling-stroke">
                          <animateMotion
                            dur="${newDur.batteryGrid}s"
                            repeatCount="indefinite"
                            calcMode="linear"
