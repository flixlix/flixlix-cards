...
import { type Box, type Connection, type ConnectionState, type EntityConfigOrStr } from "../types";
...
export function normalizeStateValue(
  ...
  const prefix =
    Object.keys(UNIT_PREFIXES).find((p) => unit_of_measurement!.indexOf(p) === 0) || "";
  const currentFactor = UNIT_PREFIXES[prefix as keyof typeof UNIT_PREFIXES] || 1;
  const targetFactor = UNIT_PREFIXES[unit_prefix as keyof typeof UNIT_PREFIXES] || 1;
  ...
  return {
    ...
    unit_of_measurement: prefix
      ? unit_of_measurement.replace(prefix, unit_prefix)
