...
export function getDefaultConfig(hass: HomeAssistant): EnergyFlowCardPlusConfig {
  ...
  const batteryEnergyEntities = powerEntities.filter((entityId) =>
    checkStrings(entityId, batteryTests)
  );
  const gridEnergyEntities = powerEntities.filter((entityId) =>
    checkStrings(entityId, gridEnergyTestString)
  );
  const firstSolarEnergyEntity = powerEntities.filter((entityId) =>
