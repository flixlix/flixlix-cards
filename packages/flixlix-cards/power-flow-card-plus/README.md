...
`entities.battery` accepts a single battery object (legacy) or an array of up to 3 battery objects. Each battery is shown as its own circle with SoC and power in/out.

Use `role: primary` (default) for packs that participate in solar/grid/home flow distribution. Use `role: satellite` for display-only packs (e.g. plug-in stations) rendered as smaller dashed circles below the primary battery; satellite power is not included in distribution math.
