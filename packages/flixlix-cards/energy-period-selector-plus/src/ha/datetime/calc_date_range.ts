...
export const calcDateRange = (
  ...
  switch (range) {
    case "today":
      return [
        calcDate(today, startOfDay, locale, hassConfig, { weekStartsOn }),
        calcDate(today, endOfDay, locale, hassConfig, { weekStartsOn }),
      ];
    case "yesterday":
      return [
        calcDate(addDays(today, -1), startOfDay, locale, hassConfig, { weekStartsOn }),
        calcDate(addDays(today, -1), endOfDay, locale, hassConfig, { weekStartsOn }),
      ];
    case "this_week":
      return [
        calcDate(today, startOfWeek, locale, hassConfig, { weekStartsOn }),
        calcDate(today, endOfWeek, locale, hassConfig, { weekStartsOn }),
      ];
    case "this_month":
      return [
    ...
    case "now-12m":
      return [
        calcDate(today, (date) => subMonths(startOfMonth(date), 11), locale, hassConfig),
        calcDate(today, endOfMonth, locale, hassConfig),
      ];
    case "now-365d":
      return [
