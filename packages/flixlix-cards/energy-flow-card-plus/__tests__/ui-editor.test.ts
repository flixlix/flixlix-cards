...
describe("energy flow ui editor", () => {
  beforeEach(() => {
  ...
  test("migrateLegacyFields maps legacy energy decimal and threshold fields", () => {
    const editor = new PowerFlowCardPlusEditor();
    ...
    expect(config.kilo_threshold).toBe(1200);
    expect(config.base_decimals).toBe(1);
    expect(config.kilo_decimals).toBe(2);
    expect(config.mega_decimals).toBe(3);
    expect(config.wh_kwh_threshold).toBeUndefined();
    expect(config.wh_decimals).toBeUndefined();
    expect(config.kwh_decimals).toBeUndefined();
    expect(config.mwh_decimals).toBeUndefined();
  });
