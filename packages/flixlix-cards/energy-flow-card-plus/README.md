...
`entities.battery` accepts a single battery object (legacy) or an array of up to 3 battery objects. Each battery is shown as its own circle with SoC and energy in/out.

Use `role: primary` (default) for packs that participate in solar/grid/home flow distribution. Use `role: satellite` for display-only packs (e.g. plug-in stations) rendered as smaller dashed circles below the primary battery; satellite energy is not included in distribution math.
...
| invert_state                     | `boolean`                                                      | `false`                                                | If set to true the direction as well as the values will be inverted, meaning a positive value will be shown as production and a negative value will be shown as consumption.                                                                                                                                                                                                                  |
| role                             | `"primary"` or `"satellite"`                                   | `primary`                                              | `primary` packs participate in solar/grid/home flow distribution. `satellite` packs are display-only (smaller dashed circles below the primary) and are excluded from distribution math.                                                                                                                                                                                                     |
