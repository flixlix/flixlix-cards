...
export const getNonFossilHas = (hass: HomeAssistant, config: EnergyFlowCardPlusConfig) => {
...
export const getNonFossilHasPercentage = (
  hass: HomeAssistant,
...
export const getNonFossilSecondaryState = (hass: HomeAssistant, config: EnergyFlowCardPlusConfig) =>
  getSecondaryState(hass, config, "fossil_fuel_percentage");

export const getNonFossilState = (hass: HomeAssistant, config: EnergyFlowCardPlusConfig) => {
  const nonFossil = config.entities.fossil_fuel_percentage;
  const fossilPercentageEntity = nonFossil?.entity;
  const gridFromGrid = getGridConsumptionState(hass, config);
