import { type PowerFlowCardPlus } from "@/power-flow-card-plus";
import { type ConfigEntities } from "@/power-flow-card-plus-config";
  ...
  if (individual?.some((ind) => ind.has)) {
    const getStylesForIndividual = (field: IndividualDeviceType, index: number) => {
      ...
      main.style.setProperty(
        ...
        individualColor || colors[index] || "#d0cc5b"
      );
    ...
    for (
      let index = 0;
      index < (individualsShown.length < 4 ? individualsShown.length : 4);
      index++
    ) {
      const individualShown = individualsShown[index];
      if (!individualShown) continue;
      getStylesForIndividual(individualShown.field as IndividualDeviceType, index);
    }
  }
};
